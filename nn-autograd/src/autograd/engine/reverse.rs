        ...
        for node in topo.into_iter().rev() {
            ...
            match &node.grad_op {
                ...
                GradOp::Tanh => {
                    ...
                    let a = GradValue::<B>::from_id(node.input_ids[0], inp_ndim);
                    let tanh = a.tanh();
                    let sech2 = tanh.clone().pow(2.).neg().add_scalar(1.0);
                    accumulate(node.input_ids[0], go.mul(&sech2), &mut grads);
                }
                ...
                GradOp::GELU => {
                    // d/da = go * (0.5 * (1 + tanh(a/sqrt(2))) + a * (1 - tanh^2(a/sqrt(2))) / (2*sqrt(2)))
                    let a = GradValue::<B>::from_id(node.input_ids[0], inp_ndim);
                    let a_div_sqrt2 = a.clone().div_scalar(2.0f32.sqrt());
                    let tanh = a_div_sqrt2.tanh();
                    let left = tanh.clone().add_scalar(1.0).mul_scalar(0.5);
                    let sech2 = tanh.pow(2.).neg().add_scalar(1.0);
                    let right = a.clone().mul(&sech2).div_scalar(2.0f32 * 2.0f32.sqrt());
                    let ga = go.mul(&left.add_into(right));
                    accumulate(node.input_ids[0], ga, &mut grads);
                }
