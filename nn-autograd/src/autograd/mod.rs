...
use crate::autograd::engine::AutogradStrategy;
...
/// Extension methods for `Tensor<Autograd<B>, NDIM>`.
///
/// This is a trait rather than an inherent impl because `Tensor` is defined in
/// `nn-core` (a foreign crate), and the orphan rule forbids adding inherent impls
/// to foreign types. The trait is local to `nn-autograd`, so the impl is allowed.
pub trait AutogradTensor<B: Backend, const NDIM: usize> {
    fn backward<S: AutogradStrategy>(&self);
    fn grad(&self) -> Option<Tensor<Autograd<B>, NDIM>>;
    fn zero_grad(&self);
    fn with_grad(data: Vec<Scalar>, shape: [usize; NDIM]) -> Self;
}

impl<B: Backend, const NDIM: usize> AutogradTensor<B, NDIM> for Tensor<Autograd<B>, NDIM> {
    fn backward<S: AutogradStrategy>(&self) {
        REQUIRES_GRAD.with(|list| {
...
/// Strip the `Autograd` wrapper: borrow a tracked tensor as a plain backend tensor.
#[inline(always)]
pub(crate) fn as_inner<B: Backend, const NDIM: usize>(
    t: &Tensor<Autograd<B>, NDIM>,
) -> Tensor<B, NDIM> {
    Tensor::from_id(t.id)
}

/// Re-wrap a plain backend result as a tracked tensor (same allocator slot, new type tag).
#[inline(always)]
pub(crate) fn wrap<B: Backend, const NDIM: usize>(
    t: Tensor<B, NDIM>,
...
impl<B: Backend> Backend for Autograd<B> {
    ...
    fn shape<const NDIM: usize>(tensor: &Tensor<Self, NDIM>) -> [usize; NDIM] {
        B::shape(&as_inner(tensor))
    }

    fn data<const NDIM: usize>(tensor: &Tensor<Self, NDIM>) -> Vec<Scalar> {
        B::data(&as_inner(tensor))
    }

    fn with_data<F, const NDIM: usize>(tensor: &Tensor<Self, NDIM>, f: F)
    where
        F: FnOnce(&[Scalar]),
    {
        B::with_data(&as_inner(tensor), f)
    }
