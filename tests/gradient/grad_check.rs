...
//!  - numerically, via central finite differences on a plain (tape-free) CPU forward,
//! then asserts the two agree within tolerance. This catches mistakes in the backward
//! formulas that shape-only assertions miss — in particular the batched/2D path.

use nn_rs::backend::autograd::engine::ReverseMode;
use nn_rs::backend::autograd::{Autograd, clear_grad_storage, clear_tape};
