...
fn test_tanh_1d() {
    ...
    for i in 0..3 {
        ...
        assert!((result.get([i]) - expected).abs() < 1e-6, "tanh({}) mismatch", data[i]);
    }
}
...
fn test_tanh_bounds() {
    ...
    let tensor = Tensor::<CPUBackend, 1>::new(data, [2]);
    let result = tensor.tanh();
    assert!((result.get([0]) - (-1.0)).abs() < 1e-6);
    assert!((result.get([1]) - 1.0).abs() < 1e-6);
}
