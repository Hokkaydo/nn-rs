use crate::backend::autograd::{Autograd, GradNode};
...
impl<B: Backend> GradValue<B> {
    ...
    pub fn as_rank1(self) -> Tensor<B, 1> {
        match self {
            Self::Rank1(t) => t,
            _ => panic!("Expected rank-1 gradient"),
        }
    }
    ...
    pub fn as_rank2(self) -> Tensor<B, 2> {
        match self {
            Self::Rank2(t) => t,
            _ => panic!("Expected rank-2 gradient"),
        }
    }
    ...
    pub fn broadcast_to_id(self, target_id: TensorId) -> Self {
        ...
        match self {
            Self::Rank1(t) => {
                ...
                // Materialize via add with zeros — B::add handles strides correctly
